...
public class StaticMesh extends BaseMesh {
    ...
    public StaticMesh(float[] vertexData, int[] indexData, int strideBytes, VertexAttribute... attributes) {
        ...
        this.vertexCount = vertexData.length / (strideBytes / 4);
    ...
    /**
     * 使用 VertexFormat 创建静态 Mesh
     *
     * @param vertexData 顶点数据
     * @param indexData  索引数据
     * @param format     顶点格式
     */
    public StaticMesh(float[] vertexData, int[] indexData, VertexFormat format) {
        super(format);
        this.elementCount = indexData.length;
        this.vertexCount = vertexData.length / format.getFloatsPerVertex();
